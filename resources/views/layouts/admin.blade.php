                ...
                <div class="user-action">
                    <div class="dropdown notification">
                        ...
                        <ul class="dropdown-menu notificationList" aria-labelledby="notificationButton">
                            @if (Auth::user()->role == 0)
                                @foreach (DB::table('notifications')->where('for', 0)->orderBy('created_at','DESC')->limit(3)->get()
    as $item)
                            ...
                            @else
                                @foreach (DB::table('notifications')->where('for', Auth::id())->orderBy('created_at','DESC')->limit(3)->get()
    as $item)
                        ...
                        <div class="notificationNumber">
                            <p class="numberAllNotification">
                                @if (Auth::user()->role == 0)
                                    {{ DB::table('notifications')->where('for', 0)->limit(3)->count() }}
                                    @else
                                    {{ DB::table('notifications')->where('for', Auth::id())->limit(3)->count() }}
                                @endif
                        </div>
                    </div>
